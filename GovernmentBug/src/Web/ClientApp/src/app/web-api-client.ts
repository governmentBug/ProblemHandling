...
export interface ICommentsClient {
    ...
    createCommentt(command: CreateCommentCommand): Observable<number>;
}
...
export class CommentsClient implements ICommentsClient {
    ...
    createCommentt(command: CreateCommentCommand): Observable<number> {
        let url_ = this.baseUrl + "/api/Comments/try";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processCreateCommentt(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processCreateCommentt(response_ as any);
                } catch (e) {
                    return _observableThrow(e) as any as Observable<number>;
                }
            } else
    ...
    protected processCreateCommentt(response: HttpResponseBase): Observable<number> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
...
export class CreateCommentCommand implements ICreateCommentCommand {
    ...
    init(_data?: any) {
        if (_data) {
            ...
            if (Array.isArray(_data["usersMentions"])) {
                this.usersMentions = [] as any;
                for (let item of _data["usersMentions"])
                    this.usersMentions!.push(item);
            }
        }
    }
    ...
    toJSON(data?: any) {
        ...
        if (Array.isArray(this.usersMentions)) {
            data["usersMentions"] = [];
            for (let item of this.usersMentions)
                data["usersMentions"].push(item);
        }
        return data;
    }
}
